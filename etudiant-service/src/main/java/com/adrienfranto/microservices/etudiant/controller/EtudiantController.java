...
public class EtudiantController {
    private final EtudiantService etudiantService; private final String uploadDir = "uploads/images";
    @PostMapping
    public ResponseEntity<EtudiantReponse> ajouterEtudiant(
            @RequestPart("etudiant") EtudiantRequest etudiantRequest,@RequestPart("image") MultipartFile imageFile) {
        return ResponseEntity.ok(etudiantService.ajouterEtudiant(etudiantRequest, imageFile));
    }
    @GetMapping
    public ResponseEntity<List<EtudiantReponse>> afficherEtudiants() { return ResponseEntity.ok(etudiantService.afficherEtudiants());
    }
    @GetMapping("/{id}")
    public ResponseEntity<EtudiantReponse> afficherEtudiantParId(@PathVariable String id) {
        return etudiantService.afficherEtudiantParId(id) .map(ResponseEntity::ok).orElse(ResponseEntity.notFound().build());
    }
    @PutMapping("/{id}")
    public ResponseEntity<EtudiantReponse> modifierEtudiant(@PathVariable String id,@RequestPart("etudiant") EtudiantRequest etudiantRequest,
